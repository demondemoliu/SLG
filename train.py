...
def valid(model, valid_dataloader, total_batch):
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            # # 保存分割效果最好的图片
            # if _IoU_mean > best_iou_mean:
            #     best_iou_mean = _IoU_mean
            #     best_image = img[0].clone().detach().cpu().numpy()



    metrics_result = metrics.mean(total_batch)

    # return metrics_result, best_image
...
def get_model_summary(model, input_size):
    ...
    total_gflops = flops / 1e9  # 转换为GFLOPs
    print(f"Total GFLOPs: {total_gflops}")
...
def train():
    ...
    # 假设输入大小为(3, 256, 256)，请根据你的实际输入大小调整
    input_size = (3, 320, 320)

    # 获取模型参数量和GFLOPs
    total_params, total_gflops = get_model_summary(model, input_size)
    ...
    for epoch in range(opt.nEpoch):
        ...
        # metrics_result, best_image = valid(model, valid_dataloader, val_total_batch)
